...
const update = (node: Node, test: (n: Node) => TestResult) => {
    ...
    if (testResult.match && index != -1) {
        ...
        const after = document.createTextNode(node.textContent.slice(index + testResult.match[0].length))
        span.innerText = node.textContent.slice(index, index + testResult.match[0].length)
